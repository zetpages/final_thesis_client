...
const StudentStatusElement = () => {

    const {center} = useContext(CenterContext);

    const StudentStatusRow = (stStatus) => {

        const descriptionPop = (
            <Popover id="popover-basic">
                <Popover.Header as="h3">Описание</Popover.Header>
                <Popover.Body>
                    {stStatus.description}
                </Popover.Body>
            </Popover>
        );
            ...
            <Card border="light" className="table-wrapper table-responsive shadow-sm student__table-wrapper with-effect">
                <Card.Body className="pt-0">
                    <Table hover className="user-table align-items-center">
                        ...
                        <tbody>
                        {center.student_statuses?.map(t => <StudentStatusRow key={t.id} {...t} />)}
                        </tbody>
                    </Table>
                </Card.Body>
            </Card>
...
export default StudentStatusElement;